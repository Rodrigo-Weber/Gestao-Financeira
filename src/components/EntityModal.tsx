  ...
  return <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <form className="modal-card entity-modal" onSubmit={(event) => { event.preventDefault(); onSave(kind, form); }}>
      ...
      {kind === "card" && <>
        ...
        <div className="form-grid"><label>Dia do fechamento<input type="number" min="1" max="31" value={field("closingDay", 10)} onChange={(e) => update("closingDay", Number(e.target.value))} required /></label><label>Dia do vencimento<input type="number" min="1" max="31" value={field("dueDay", 17)} onChange={(e) => update("dueDay", Number(e.target.value))} required /></label></div>
      </>}
      {kind === "debt" && <>
