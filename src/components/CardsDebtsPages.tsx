...
export function DebtsPage({ data, onAdd }: { data: FinanceData; onAdd: () => void }) {
  ...
  return <div className="page-stack"><section className="page-title"><div><span className="eyebrow">Plano de liberdade</span><h1>Dívidas e empréstimos</h1><p>Priorize pagamentos e veja quando cada compromisso termina.</p></div><button className="primary-btn" onClick={onAdd}><Plus size={18} /> Nova dívida</button></section>
    ...
    <section className="debt-layout"><div className="panel debt-list"><div className="panel-heading"><div><span className="eyebrow">Compromissos</span><h2>Suas dívidas</h2></div></div>{data.debts.map((debt) => {
      const paidPercent = debt.originalAmount > 0 ? Math.max(0, Math.min(100, Math.round((1 - debt.outstandingBalance / debt.originalAmount) * 100))) : 0;
      return <div className="debt-row" key={debt.id}><span className="round-icon soft">{debt.type === "person" ? <UserRound size={19} /> : <Landmark size={19} />}</span><div className="debt-main"><strong>{debt.name}{debt.source === "pluggy" && <span className="source-tag">Pluggy</span>}</strong><small>{debt.creditor} • vence dia {debt.dueDay}{debt.contractEndDate ? ` • termina ${new Date(`${debt.contractEndDate}T12:00:00`).toLocaleDateString("pt-BR")}` : ""}</small><div className="progress"><span style={{ width: `${paidPercent}%` }} /></div><span>{paidPercent}% quitado{debt.remainingInstallments ? ` • ${debt.remainingInstallments} parcelas restantes` : ""}</span></div><div className="debt-values"><small>Saldo</small><strong>{brl.format(debt.outstandingBalance)}</strong><span>{debt.monthlyInterest}% a.m.{debt.annualCet ? ` • CET ${debt.annualCet}% a.a.` : ""}</span><small>Parcela estimada {brl.format(debt.minimumPayment)}</small></div></div>;
    })}{!data.debts.length && <div className="empty-state"><Landmark size={24} /><strong>Nenhuma dívida ativa</strong><span>Adicione manualmente ou sincronize a Pluggy.</span></div>}</div>
